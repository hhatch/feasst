...
def slurm_single_node(params):
    ...
    params['print_queue_id'] = "sleep 2; squeue -u $USER | tail -1| awk '{print $1}' | awk -F '_' '{print $1}'"
...
def run_simulations(params, sim_node_dependent_params, write_feasst_script, post_process, queue_function, args):
    ...
    if args.run_type == 0: # run directly
        ...
        for node in range(params['num_nodes']):
            ...
            subprocess.check_call(params['queue_command'], shell=True, executable='/bin/bash')
            subprocess.check_call(params['print_queue_id'] + " >> " + queue_id_file, shell=True, executable='/bin/bash')

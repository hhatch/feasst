...
int main() {  // PatchKF, REFCONF_REINHARDT
  ...
  xAdd[0] = 1.01 - space->x(2, 0);
  space->transMol(1, xAdd);
  pair->initEnergy();
  ASSERT(fabs(pair->peTot() + 0.99990920632356173) < feasst::DTOL,
    "SQW1: " << MAX_PRECISION << pair->peTot());
  pair->updateClusters(0.);
  ...
  space->transMol(1, xAdd);
  pair->initEnergy();
  ASSERT(pair->peTot() < feasst::DTOL, "SQW2");
  pair->updateClusters(0.);

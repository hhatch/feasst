...
mc.add(fst.MakeCheckEnergy(fst.args({"trials_per_update" : "1e5", "tolerance" : "1e-8"})))
mc.add(fst.MakeTune())
mc.run(fst.MakeRun(fst.args({"num_trials": "1e5"})))

# nvt production
mc.add(fst.MakeLog(fst.args({"trials_per_write" : "1e5", "file_name" : "lj.csv"})))
mc.add(fst.MakeMovie(fst.args({"trials_per_write" : "1e5", "file_name" : "lj.xyz"})))

...
class MapEnergyMapNeighborMW {
 public:
  MapEnergyMapNeighborMW() {
    EnergyMapNeighborMW().deserialize_map()["EnergyMapNeighborMW"] =
      MakeEnergyMapNeighborMW();
  }
};

static MapEnergyMapNeighborMW mapper_ = MapEnergyMapNeighborMW();

EnergyMapNeighborMW::EnergyMapNeighborMW(const argtype& args) : EnergyMap(args) {
  class_name_ = "EnergyMapNeighborMW";
}

EnergyMapNeighborMW::EnergyMapNeighborMW(std::istream& istr) : EnergyMap(istr) {
  const int version = feasst_deserialize_version(istr);
  ASSERT(version == 2947, "mismatch:" << version);
}

void EnergyMapNeighborMW::serialize_energy_map_neighbor_mw_(std::ostream& ostr) const {
  serialize_energy_map_(ostr);
  feasst_serialize_version(2947, ostr);
}

void EnergyMapNeighborMW::serialize(std::ostream& ostr) const {
  serialize_energy_map_neighbor_mw_(ostr);
}

std::vector<double> * EnergyMapNeighborMW::smap_(const int part1_index,
...
std::vector<double> * EnergyMapNeighborMW::smap_new_(const int part1_index,
    const int site1_index,
    const int part2_index,
    const int site2_index) {
  FATAL("not implemented");
...
double EnergyMapNeighborMW::update(
    const double energy,
...
void EnergyMapNeighborMW::size_map_() {
...
void EnergyMapNeighborMW::remove_from_map_nvt_(const Select& select) {
...
void EnergyMapNeighborMW::add_to_map_nvt_() {
...
void EnergyMapNeighborMW::remove_particle_from_map_(const Select& select) {
...
void EnergyMapNeighborMW::add_particle_to_map_() {
  // Add "new" to map
  DEBUG("map new size: " << map_new_()->size());
  for (const std::pair<int, mn4type>& mn4 : *map_new_()) {
    const int part1 = mn4.first;
...
const map3type * EnergyMapNeighborMW::find_map3_(const int particle_index1,
...
const map2type * EnergyMapNeighborMW::find_map2_(const int part1,
    const int site1,
    const int part2,
    const bool old) const {
...
bool EnergyMapNeighborMW::is_cluster_(const NeighborCriteria& neighbor_criteria,
...
bool EnergyMapNeighborMW::is_cluster_changed(
    const NeighborCriteria& neighbor_criteria,
    const Select& select,
...
void EnergyMapNeighborMW::resize_(
    const int part1_index,
    const int site1_index,
    const int part2_index,
    const int site2_index) { FATAL("not impl"); }

double EnergyMapNeighborMW::total_energy() const {
  double en = 0.;
...
void EnergyMapNeighborMW::check() const {
...
void EnergyMapNeighborMW::neighbors(
    const NeighborCriteria& neighbor_criteria,
    const Configuration& config,
...
std::string EnergyMapNeighborMW::map_str(const map3type& map3) const {
  std::stringstream ss;
  for (const std::pair<int, map2type>& map2 : map3) {
    ss << "p2 " << map2.first << ":{";
    for (const std::pair<int, map1type>& map1 : map2.second) {
...
std::string EnergyMapNeighborMW::map_new_str() const {
  std::stringstream ss;
  ss << std::endl;
...
std::string EnergyMapNeighborMW::map_str() const {
  std::stringstream ss;
  ss << std::endl;
  for (int part1 = 0; part1 < static_cast<int>(const_map_().size()); ++part1) {
    ss << "p1 " << part1 << ":|";
...
const std::vector<std::pair<int, mn4type> >& EnergyMapNeighborMW::const_map_new_() const {

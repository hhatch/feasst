...
#ifndef FEASST_CLUSTER_ENERGY_MAP_NEIGHBOR_MW_H_
#define FEASST_CLUSTER_ENERGY_MAP_NEIGHBOR_MW_H_
...
/**
  Similar to EnergyMapNeighbor, except modified for use by Mat Witman.
  Differences will include:
  - unique data structure. neighbor[particle index][neighbor][0 - type; 1 - distance; 2 - neighbor particle index]
 */
class EnergyMapNeighborMW : public EnergyMap {
 public:
  EnergyMapNeighborMW(const argtype& args = argtype());
  ...
  std::shared_ptr<EnergyMap> create(std::istream& istr) const override {
    return std::make_shared<EnergyMapNeighborMW>(istr); }
  void serialize(std::ostream& ostr) const override;
  EnergyMapNeighborMW(std::istream& istr);
  virtual ~EnergyMapNeighborMW() {}

 protected:
  void serialize_energy_map_neighbor_mw_(std::ostream& ostr) const;
...
inline std::shared_ptr<EnergyMapNeighborMW> MakeEnergyMapNeighborMW(
    const argtype& args = argtype()) {
  return std::make_shared<EnergyMapNeighborMW>(args);
}

}  // namespace feasst

#endif  // FEASST_CLUSTER_ENERGY_MAP_NEIGHBOR_MW_H_

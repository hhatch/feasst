...
//  EXPECT_EQ(config->particle(0).site(0).num_cells(), 2);
  ...
  visit->finalize(select, config.get());
...
//  config->init_cells(3);
  ...
  visit.check_energy(&model, config.get());
  cell_visit->check_energy(&model, config.get());

  DEBUG("Move to the same cell");
  pos.set_vector({-1, 1, 1});
  config->displace_particle(select, pos);
  cell_visit->finalize(select, config.get());
  EXPECT_EQ(config->particle(0).site(0).cell(0), center);
  ...
  model.compute(select, config.get(), &visit);
  model.compute(select, config.get(), cell_visit.get());
  EXPECT_NEAR(visit.energy(), pe_lj, NEAR_ZERO);
